...
export const AppProvider = ({ children }: PropsChildren) => {
  ...
  const [searchValue, setSearchValue] = useState(
    localStorage.getItem("searchValue") || "harry potter"
  );
  const [numPageAtual, setNumPageAtual] = useState(
    parseInt(localStorage.getItem("numPageAtual") || "1", 10)
  );
  ...
  const filteredFavoritos = dados?.Search?.filter((item) =>
    favoritesFromStorage.includes(item.Title)
  );
