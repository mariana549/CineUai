...
describe('Função getApi_Dados', () => {
    ...
    it('testando se retorna os resultados coretamente', async () => {
        const ID = "tt0926084";
        const PLOT = "full";
        mock.onGet(`https://www.omdbapi.com/?i=${ID}&plot=${PLOT}&apikey=${CHAVE_API_KEY}`)
    ...
    it('deve lidar corretamente com dados não existentes ou com ID inválido', async () => {
        ...
        await expect(getApi_Dados(ID, PLOT)).rejects.toThrow('Request failed with status code 404');
      });
})